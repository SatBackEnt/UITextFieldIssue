 ...
 *  - You can add Item objects in runtime using addChild:<br/>
 *  - But the only accepted children are node objects</p>
 * @class
 * @extends cc.Layer
 * @param {...cc.node
 * @example
 * var layer = new FormLayout(item1, item2, item3);
 */
FormLayout = cc.LayerColor.extend(/** @lends FormLayout# */{
//    enabled: false,
//
//    _selectedItem: null,
//    _state: -1,
//    _touchListener: null,
    _className: "FormLayout",

    /**
     * Constructor of FormLayout override it to extend the construction behavior, remember to call "this._super()" in the extended "ctor" function.
     * @param {... FormLayout|null} formItems
     */
    ctor: function (formItems) {
        cc.LayerColor.prototype.ctor.call(this);  //cc.Layer.prototype.ctor.call(this);
...
//    },
    ...
    /**
     * initializes a FormLayout with it's items
     * @param {Array} args
     * @return {Boolean}
     */
    ...
    initWithArray: function (arrayOfItems) {
        if (cc.LayerColor.prototype.init.call(this)) { //cc.Layer.prototype.init.call(this)
            //this.enabled = true;

            // menu in the center of the screen
            var winSize = cc.winSize;
            //this.setPosition(winSize.width / 2, winSize.height / 2);
            this.setContentSize(cc.size(winSize.width, winSize.height/4));
...
//            throw new Error("cc.Menu.addChild() : Menu only supports MenuItem objects as children");
        cc.LayerColor.prototype.addChild.call(this, child, zOrder, tag); //cc.Layer.prototype
    },
    ...
    alignItemsTest: function (padding) {
        this._align = 'horizontally';
        var width = -padding, locChildren = this._children, i, len, locScaleX, locWidth, locChild;
        if (locChildren && locChildren.length > 0) {
            for (i = 0, len = locChildren.length; i < len; i++)
            ...
            for (i = 0, len = locChildren.length; i < len; i++) {
                ...
                locScaleX = locChild.scaleX;
                locWidth = locChildren[i].width;
//                locChild.setPosition(x + locWidth * 1 / 2, 400);
                locChild.setPosition( 100+100*i, 200);
                x += locWidth * locScaleX + padding;
            }
        }
    },
    ...
    /**
     * align layout items in rows
     * @param {Number}
     * @example
     * // Example
     * layout.alignItemsInRows(5,3)//this will align items to 2 rows, first row with 5 items, second row with 3
     *
     * layout.alignItemsInRows(4,4,4,4)//this creates 4 rows each have 4 items
     */
    alignItemsInRows: function (/*Multiple arguments*/) {
        ...
        // check if too many rows/columns for available layout items.
 ...
 * @param {...cc.MenuItem|null} formItems
 * todo: need to use new
 * @return {cc.Menu}
 */
FormLayout.create = function (formItems) {
    ...
    if (argc === 0)
        ...
        ret = new FormLayout(formItems);
    else
        ret = new FormLayout(Array.prototype.slice.call(arguments, 0));
    return ret;
};